...
runs <- list.files("../cmd/primer_cutted/bacteria")
...
for (run in runs) {
  ...
  path <- paste("../cmd/primer_cutted/bacteria/", run, sep="")
  fnFs <- sort(list.files(path, pattern="r1_cutted.fastq", full.names = TRUE))
  fnRs <- sort(list.files(path, pattern="r2_cutted.fastq", full.names = TRUE))
...
#rename rownames
rownames(seqtab.nochim) <- gsub("-1193R_(F|R)_filt.fastq","",rownames(seqtab.nochim))
rownames(seqtab.nochim) <- gsub("-799F-runBE[0-9]{2}","",rownames(seqtab.nochim))
