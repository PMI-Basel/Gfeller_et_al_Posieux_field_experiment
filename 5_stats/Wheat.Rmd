...
#colors
DESIGN$cols_Conditioning <- NA
DESIGN$cols_Conditioning[DESIGN$Conditioning=="BX+"] <- "goldenrod1"
DESIGN$cols_Conditioning[DESIGN$Conditioning=="BX-"] <- "palegreen1"
...
bdist_paov_root <- adonis2(bdist_root ~ Conditioning * Variety + Run, data=bDESIGN_root) #wo species_varieties
...
bdist_paov_rhizosphere <- adonis2(bdist_rhizosphere ~ Conditioning * Variety + Run, data=bDESIGN_rhizosphere) #wo species_varieties
...
bdist_paov_soil <- adonis2(bdist_soil ~ Conditioning * Variety + Run, data=bDESIGN_soil) #wo species_varieties
...
fdist_paov_root <- adonis2(fdist_root ~ Conditioning * Variety + Run, data=fDESIGN_root) #wo species_varieties
...
fdist_paov_rhizosphere <- adonis2(fdist_rhizosphere ~ Conditioning * Variety + Run, data=fDESIGN_rhizosphere) #wo species_varieties
...
fdist_paov_soil <- adonis2(fdist_soil ~ Conditioning * Variety + Run, data=fDESIGN_soil) #wo species_varieties
pander(fdist_paov_soil, caption="Fungi PERMANOVA: soil")

```

### Figure 1 \| CAP - Conditioning

\vspace{5mm}

```{r CAP Conditioning compartments, echo=F, warning=F, eval=T, fig.height=7, fig.width=6}
...
#root
CAP_bray <- ordinate(bPHYSEQ_root, method="CAP", ~ Conditioning * Variety + Run, distance="bray") #CAP
...
#plot
p0 <- phyloseq::plot_ordination(phy_sub, CAP_bray, color="Conditioning", title =  "Root bacteria", axes=c(1,2))
p0$layers <- p0$layers[-1]
p0 <- p0 + geom_point(size=3, shape=16)
p0 <- p0 + scale_color_manual(values=level_cols_Conditioning)
p0 <- p0 + scale_size_manual(values=level_cols_Conditioning)
p0 <- p0 + theme_bw()
fig_bac_root <- p0


#rhizosphere
CAP_bray <- ordinate(bPHYSEQ_rhizosphere, method="CAP", ~ Conditioning * Variety + Run, distance="bray") #CAP
CAP_bray_var_tbl <- variability_table(CAP_bray) # extract variation details
...
#plot
p0 <- phyloseq::plot_ordination(phy_sub, CAP_bray, color="Conditioning", title =  "Rhizosphere bacteria", axes=c(1,2))
p0$layers <- p0$layers[-1]
p0 <- p0 + geom_point(size=3, shape=16)
p0 <- p0 + scale_color_manual(values=level_cols_Conditioning)
p0 <- p0 + scale_size_manual(values=level_cols_Conditioning)
...
#root
CAP_bray <- ordinate(fPHYSEQ_root, method="CAP", ~ Conditioning * Variety + Run, distance="bray") #CAP
CAP_bray_var_tbl <- variability_table(CAP_bray) # extract variation details
...
#plot
p0 <- phyloseq::plot_ordination(phy_sub, CAP_bray, color="Conditioning", title =  "Root fungi", axes=c(1,2))
p0$layers <- p0$layers[-1]
p0 <- p0 + geom_point(size=3, shape=16)
p0 <- p0 + scale_color_manual(values=level_cols_Conditioning)
p0 <- p0 + scale_size_manual(values=level_cols_Conditioning)
...
#rhizosphere
CAP_bray <- ordinate(fPHYSEQ_rhizosphere, method="CAP", ~ Conditioning * Variety + Run, distance="bray") #CAP
CAP_bray_var_tbl <- variability_table(CAP_bray) # extract variation details
...
#plot
p0 <- phyloseq::plot_ordination(phy_sub, CAP_bray, color="Conditioning", title =  "Rhizosphere fungi", axes=c(1,2))
p0$layers <- p0$layers[-1]
p0 <- p0 + geom_point(size=3, shape=16)
p0 <- p0 + scale_color_manual(values=level_cols_Conditioning)
p0 <- p0 + scale_size_manual(values=level_cols_Conditioning)
...
#add legend
legend <- get_legend(fig_bac_root + theme(legend.position="top") + labs(fill="Wheat Conditioning"))
...
#sort
bPHYSEQ_phyla_melt <- bPHYSEQ_phyla_melt[order(bPHYSEQ_phyla_melt$Conditioning),]
...
fig2.1 <- ggplot(bPHYSEQ_phyla_melt, aes_string(x="SampleID", y="Abundance", fill="labels_2")) + 
  ...
  geom_text(data=bPHYSEQ_phyla_melt, aes(label=Conditioning, y=-5), angle=90, check_overlap=T, size=3, hjust = 0.25) + 
  theme_bw()+
...
#sort
fPHYSEQ_phyla_melt <- fPHYSEQ_phyla_melt[order(fPHYSEQ_phyla_melt$Conditioning),]
...
fig2.2 <- ggplot(fPHYSEQ_phyla_melt, aes_string(x="SampleID", y="Abundance", fill="labels_2")) + 
  ...
  guides(fill=guide_legend(title="Phylum"))+
  facet_grid( ~ SampleType, scale="free_x", space="free_x") +
  geom_text(data=fPHYSEQ_phyla_melt, aes(label=Conditioning, y=-5), angle=90, check_overlap=T, size=3, hjust = 0.25) + 
  theme_bw()+
